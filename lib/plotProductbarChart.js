...
var filterValueOfState = 'Vermont';
var filterData;
var filterDataState;
function renderProductBarChart(svgId) {
    ...
    const data =  d3.csv('data/markets.csv').then(function(data) {
        ...
        const productData =  d3.csv('data/products.csv').then(function(productData) {
          var filteredProductData = productData.filter(function(d) {
            var fmid = d.FMID;
            return filterData.filter(d => d.FMID == fmid).length > 0;
          });
          var filteredProductDataForPlot = [];
          console.log(filteredProductData);
          if(filteredProductData[0]) {
            productList = Object.keys(filteredProductData[0]).filter(d => d != "FMID");
            productList.forEach(element => {
              filteredProductDataForPlotCount = d3.nest()
                ...
                .entries(filteredProductData);
              var item = {
                key: element,
                value: filteredProductDataForPlotCount.filter(d => d.key == 'Y')[0].value
              };
        ...
        if(filterData) {
          var table = d3.select('#product-table-chart-svg').attr("class", "table")
                    .attr("width", WIDTH)
                    .attr("height", HEIGHT);
