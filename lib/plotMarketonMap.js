  ...
  var marketData = d3.csv('data/markets.csv').then(function (marketData) {
    ...
    var tip = d3.tip().attr('class', 'd3-tip')
      .html(function (usMapDataForAState) {
        ...
        var content = "<span style='margin-left: 2.5px;'><h6>State Name: " + usMapDataForAState.properties.name + "</h6></span><br>";
        content += "<span style='margin-left: 2.5px;'><h6>No. Of Markets : " + marketCount + "</h6></span><br>";
        return content;
      });
    svg.call(tip);
    //Setting geoPath
    ...
    d3.json("data/us-10m.v2.json").then(function (us) {
      ...
      var annotations = [{
        note: {
          align: "right",
          label: "Most number of Farmers Market: 756",
          title: "California"
        },
        x: 50, y: 250,
        dy: -20,
